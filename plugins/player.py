...
async def add_to_playlist(_, message: Message):
    if Config.ADMIN_ONLY == "Y":
        ...
        if message.from_user.id not in admins:
            await message.reply_sticker("CAACAgUAAxkBAAIB52E-QTe8RbGgdg58LfKqJyf594zNAALIAwACucGRVfmUqyw55GCyIAQ")
            return
    type=""
    yturl=""
    ysearch=""
...
async def yt_play_list(client, m: Message):
    if m.reply_to_message is not None and m.reply_to_message.document:
        if m.reply_to_message.document.file_name != "YouTube_PlayList.json":
            await m.reply("Invalid PlayList file given. Use  or search for a playlist in to get a playlist file.")
            return
...
async def notforu(_, m: Message):
    await _.send_cached_media(chat_id=m.chat.id, file_id="CAACAgUAAxkBAAIB52E-QTe8RbGgdg58LfKqJyf594zNAALIAwACucGRVfmUqyw55GCyIAQ", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️Join Here', url='https://t.me/subin_works')]]))
...
async def not_chat(_, m: Message):
    buttons = [
        [
            InlineKeyboardButton('🧩 Feedback', url='https://t.me/owohub'),
        ]
        ]
